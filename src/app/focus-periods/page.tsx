...
export default function FocusPeriodsPage() {
  ...
  const openCopyModal = (period: FocusPeriod) => {
    setEditingPeriod(null); // Set to null so it creates a new period
    setNewPeriod({
      name: `Copy of ${period.name}`,
      start_date: period.start_date.split('T')[0], // Format for date input
      end_date: period.end_date.split('T')[0],
      capacity_model: period.capacity_model,
      azdo_iteration_path: period.azdo_iteration_path || '',
          ...
          <div key={period.id} className="card">
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <button
                  onClick={() => openCopyModal(period)}
                  className="text-purple-600 hover:text-purple-800"
                  title="Copy focus period"
                >
